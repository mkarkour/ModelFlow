...
> [!NOTE]
> 🆘 Need Help?
> If you feel lost with dbt commands or concepts (no shame in that!), a quick summary of the most common commands and core concepts can be found in the [dbt CheatSheet](dbt_cheatsheet.md).
...
> If you want to see the final SQL code generated by dbt (with Jinja templates, macros, and dbt packages fully resolved), check the `target/` directory:
> * **Compiled SQL:** Look in `target/compiled/modelflow/models/...` to see the `SELECT` statements.
> * **Run SQL:** Look in `target/run/modelflow/models/...` to see the full DDL (e.g., `CREATE TABLE AS...`) sent to DuckDB.
> 
> These files are updated every time you run `dbt compile`, `dbt run`, or `dbt build`.
...
🏁 Final Conclusion
Once you have finished all layers (Bronze, Silver, Gold) for the three architectures, head over to the [Modeling Walkthrough](modeling_walkthrough.md) file.

This is the capstone reference. It provides a synthesis of the journey, compares the trade-offs of each methodology (Auditability vs. Performance), and provides a framework for choosing the right architecture in your future projects.
